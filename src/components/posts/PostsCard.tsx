...
export default function PostsCard({ post }: { post: Post }) {
  ...
  return (
    <article className="text-center transition-all shadow-lg hover:-translate-y-1 hover:shadow-xl">
      <Link href={`/posts/${path}`} className="">
        ...
        <p className="my-2">
          <button className="px-2 py-1 rounded-lg bg-emerald-300">{category}</button>
        </p>
      </Link>
    </article>
  );
